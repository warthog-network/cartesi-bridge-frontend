...
export function describeRollupInput(payload, opts = {}) {
  ...
  const titles = {
    ...
    pool_deposit: 'Path A: credit pool from Warthog deposit proof',
    pool_deposit_rejected: 'Path A: pool deposit rejected (should not be sent)',
    pool_mint_wwart: 'Path A: mint wWART claim against shared pool',
    pool_withdraw_wwart: 'Path A: L1 mint voucher (pool claim)',
    pool_burn_wwart: 'Path A: burn pool wWART claim',
    pool_redeem: 'Path A: redeem WART from pool (release ticket)',
